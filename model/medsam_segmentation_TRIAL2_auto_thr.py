...
def _torch_load_compat(path: str, map_location: str | torch.device = "cpu"):
    """
    PyTorch 2.6 changed torch.load default to weights_only=True.
    For trusted internal checkpoints, force weights_only=False.
    """
    try:
        return torch.load(path, map_location=map_location, weights_only=False)
    except TypeError:
        # Older torch versions do not support weights_only arg
...
def _load_medsam_sam_model(sam_type: str, ckpt_path: str, device: torch.device):
    ...
    try:
        ...
        state = _torch_load_compat(ckpt_path, map_location="cpu")
...
def load_ckpt(
    ...
    ckpt = _torch_load_compat(path, map_location="cpu")
